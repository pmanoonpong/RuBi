...
\item For each couple until $(F_{min}, t_{max})$, obtained from \ref{eq:work}, compute the torques $\tau_{i,j}$ for each joint through \ref{eq:dynamics_eq1} and $\theta_{i,j}$ as in \ref{eq:joint_vel}. \footnote{Eq. \ref{eq:joint_vel} introduced the assumption that the joint velocities are constant for simplicity}
