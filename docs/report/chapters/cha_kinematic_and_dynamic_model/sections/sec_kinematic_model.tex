...
The variables $x_{i}, y_{i}$ represent the coordinates of the joints, named $P_{i}$ in the schematic.
Once computed, the linear velocity and acceleration of every joint can be obtained by derivation of the forward kinematic model.
Following the same approach, the inverse kinematic model can be computed for one leg, yielding equation \ref{eq:inverse_kinematics}.
...
And equivalently to the linear case, the inverse kinematic model can be derived to obtain the velocities and accelerations in the joint space.
