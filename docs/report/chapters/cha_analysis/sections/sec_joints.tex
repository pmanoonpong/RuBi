...
Several models from literature have been subject to a comparative analysis for their use in RuBi, being the main ones shown in Figure \ref{fig:compliance_series} for the ankle actuation (equivalent for the knees).
...
This direct connection act as a parallel spring and has the advantage of being a very flexible systems in term of changing the physical parameters of the spring. 
However its design is discarded in favor of the rotational due to the previous election of the rotational series spring.
% paragraph pea_1 (end)
...
It also has the advantage of having common interfaces which would lead to the use of the same spring in the series and parallel configuration, saving thus money and time.
...
This causes a general improvement of the system due to if smaller motors are needed, the weight will be less and thus the structure can be also reduced.
